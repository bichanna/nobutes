...
function Notes() {
  const [notes] = createResource(async () => await invoke('get_notes'));
  if (notes.error) {
    // Handle this
  }
  ...
  return <div class="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-6">
    <For each={notes()}>
      {(note, _) => (
        <A href={"/note/" + note.id} class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
          ...
          <p class="font-normal text-gray-700 dark:text-gray-400">{truncateNoteBody(note.body)}</p>
        </A>
      )}
    </For>
  </div>;
